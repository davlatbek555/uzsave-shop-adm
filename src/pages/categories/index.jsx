    ...
    },
    {
      ...
      render: (item) => (
        <Switch
          ...
          disabled={recoommendedCategories.length >= 4 && !recoommendedCategories.find(el => el.id === item.id)}
        />
      )
    },
        ...
        <Row gutter={[16, 16]}>
          <Col span={8} style={{ borderRight: '1px solid #f0f0f0' }}>
            <Form onFinish={handleFinish} layout='vertical' form={form}>
              ...
              <Form.Item label="Rasm" name='image' rules={[{
                type: 'string',
                required: true,
                message: 'URL Emas!',
                validator: (_, value) => {
              ...
              <Form.Item label="Do'konda bor yoki yo'q" name='isRecommended' rules={[{
                type: 'boolean',
                required: false,
                message: 'Maydon Bo\'sh'
              }]}>
              ...
              <FormItem>
                <Space>
                  <Button type='primary' htmlType='submit' loading={loading}>{isUpdate ? 'O\'zgartirish' : 'Yaratish'} Kategoriyani</Button>
                  {isUpdate && <Button onClick={handleCancel}>Orqaga</Button>}
                </Space>
              </FormItem>
            </Form>
          </Col>
